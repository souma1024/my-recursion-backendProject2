...
# まず、必要なモジュールをインポートし、ソケットオブジェクトを作成して、アドレスファミリ（AF_INET）とソケットタイプ（SOCK_STREAM）を指定します。
# サーバのアドレスは、任意のIPアドレスからの接続を受け入れるアドレスである0.0.0.0に設定し、サーバのポートは9001に設定されています。
sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)#SOCK_STREAMはTCP通信用のソケットタイプ
...
while True:
    ...
    try:
        ...
        filename_length = int.from_bytes(header[:1], "big")#[num1:num2]はスライス,文字列のindex:num1からnum2を切り取る
        json_length = int.from_bytes(header[1:3], "big")#int.from_bytesメソッドは第一引数にバイナリデータ、第二引数はビッグエンディアンかリトルエンディアンを選択できる
        data_length = int.from_bytes(header[4:8], "big")#バイナリデータを整数にしている
        stream_rate = 4096
        ...
        # 次に、コードはクライアントから受け取ったファイル名で新しいファイルをtempフォルダに作成します。このファイルは、withステートメントを使用してバイナリモードで開かれ、write()関数を使用して、クライアントから受信したデータをファイルに書き込みます。
        # データはrecv()関数を使用して塊単位で読み込まれ、データの塊を受信するたびにデータ長がデクリメントされます。
        # w+は終了しない場合はファイルを作成し、そうでない場合はデータを切り捨てます
        with open(os.path.join(dpath, filename), "wb+") as f:   #os.path.join(dir1, dir2, dir3, ... , file)とすると dir1/dir2/dir3/..../fileというパスを指定できる (wb+)の(b)は,バイナリモードで開く
            while data_length > 0:
